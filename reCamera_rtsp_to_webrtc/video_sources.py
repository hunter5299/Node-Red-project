...
class BaseVideoSource(VideoStreamTrack, ABC):
    ...
    def __init__(self, url: str, **kwargs):
        ...
        #
        # 可通过环境变量 FRAME_DELAY_MS 调整延迟量。
        # 调参方向（以"画面里框与目标是否对齐"为准）：
        #   - 画框超前于画面（框比目标先出现/框太新）→ 增大 FRAME_DELAY_MS（取更旧的检测结果）
        #   - 画框滞后于画面（框比目标慢半拍/框太旧）→ 减小 FRAME_DELAY_MS
        # 注意：实际延迟取决于 RTSP 传输延迟 + MQTT/推理延迟，不同网络/设备差异很大。
        #       默认 1000ms（适配较慢的 RTSP 链路，解决"框早于图像"的问题）。
        self.frame_delay_ms = int(os.environ.get("FRAME_DELAY_MS", "1300"))
